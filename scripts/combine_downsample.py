...
def main(args):
	...
	with open(args.output_file, 'w') as out:
		out.write('Strain\tReplicate\tSeed\tSize\tIntron\tMature\tPremature\n')
		for sample in sorted(samples):
			info = sample.split('_')
			for feature in sorted(features):
				out.write('%s\t%s\t%s\t%s\t%s\t%d\t%d\n' % (info[0], info[1], info[2], info[3], feature, mature[(sample, feature)], premature[(sample, feature)]))
